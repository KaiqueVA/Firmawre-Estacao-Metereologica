...
#define DONE_ALL (WIND_SPEED_BIT | WIND_DIRECTION_BIT)
...
void init_wind_sensors(void)
{
    ...
    q_wind_data = xQueueCreate(1, sizeof(wind_sensor_data_t));
    q_wind_direction = xQueueCreate(1, sizeof(float));
    q_wind_speed = xQueueCreate(1, sizeof(float));


    xTaskCreate(task_wind_speed, "task_wind_direction", 4096, NULL, 2, &task_wind_direction_handle);
    xTaskCreate(task_wind_direction, "task_wind_direction", 4096, NULL, 4, &task_wind_speed_handle);
...
void task_data_processing(void *pvArgs)
{
    wind_sensor_data_t wind_sensor;
    while(1)
    {
        xEventGroupWaitBits(eg_sync, DONE_ALL, pdTRUE, pdTRUE, portMAX_DELAY);
        xQueueReceive(q_wind_direction, &(wind_sensor.wind_direction), 0);
        xQueueReceive(q_wind_speed, &(wind_sensor.wind_speed_kmh), 0);
        ESP_LOGI("WIND_SENSOR", "Wind Speed: %.2f km/h | %.1f degree", wind_sensor.wind_speed_kmh, wind_sensor.wind_direction);
        vTaskDelay(100);
    }
}
